...
from reber_grammar import Reber_Grammar
...
parser.add_argument('reber_strings', action='store', nargs='?', default='mix', type=str, help='')
...
parser.add_argument('-cc', '--cols_per_char', dest='columns_per_char', action='store', n_args='?', const=32,
                    default=32, help='')
...
if __name__ == '__main__':
    ...
    if args.reber_strings == 'mix':
        ...
        save_path = os.path.join(project_root, 'data', 'reber_strings_dataset', 'mix')
        if not os.path.exists(save_path):
            os.mkdir(save_path)
    elif '.json' in args.reber_strings:
        with open(os.path.join(project_root, 'configs', args.reber_strings), 'r') as strings:
            reber_strings = json.load(strings)
        save_path = os.path.join(project_root, 'data', 'reber_strings_dataset', 'non_mix')
        if not os.path.exists(save_path):
            os.mkdir(save_path)
    else:
        raise ValueError('Please provide a valid file format (.json only) containing a dictionary with input-output'
                         'reber strings to be generated for the input data stream.')
    ...
    if args.save_to_disk:
        ...
        if args.reber_strings == 'mix':
            ...
            np.save(arr=rg_inputoutput, file=os.path.join(save_path, file_name + '.npy'))
        else:
