...
- The interface is defined using the Reber_Grammar() class.
...
from graphs_config import dict_reber_graphs, list_graphs, chars, embedded_chars
from grammar_utils import reberstring_length, get_graph_allPossibleTransitions
...
class Reber_Grammar:
    ...
    def generateSequences(self,
        ...
        while True:
            ...
            if minLength <= len(inchars) <= maxLength:
                return inchars, outchars
            else:
                continue

    def get_n_srg(self, n=1, minLength=5, maxLength=1000):
        """
    ...
    def get_n_erg(self, n=1, minLength=5, maxLength=1000):
        """
    ...
    def generate_RGInputOutput(self, do_ERG, nof_strings, reber_strings,
                               string_max_length=1000, sort_by_length=False):

        if reber_strings != 'mixed':
            rg_inputoutput = []
            ...
            if do_ERG:
                if sort_by_length:
                    rg_inputoutput = sorted(self.get_n_erg(n=nof_strings, maxLength=string_max_length),
                                            key=reberstring_length)
                else:
                    rg_inputoutput = self.get_n_erg(n=nof_strings, maxLength=string_max_length)
            else:
                if sort_by_length:
                    rg_inputoutput = sorted(self.get_n_srg(n=nof_strings, maxLength=string_max_length),
                                            key=reberstring_length)
                else:
