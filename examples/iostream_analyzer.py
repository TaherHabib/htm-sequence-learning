...
parser.add_argument('rg_iostream', action='store', default=None, type=str,
                    help='A .npy file containing the input stream of HTM model. See examples/reber_grammar_generator.py'
                         ' for details on how to generate input stream.')
parser.add_argument('-p', '--print_all', dest='print_all', action='store', nargs='?', const=True, default=False,
                    help='Whether to print all the reber strings in the input stream. Specifying only the short flag '
                         'without any argument leads to a value of True.')
parser.add_argument('-m', '--make_plots', dest='make_plots', action='store', nargs='?', const=True, default=False,
                    help='Whether to generate plots of distributions of the lengths of the reber strings in the input '
                         'stream as well as the transitions (trigrams) in the input stream. These are saved in figures/stats'
                         ' directory. Specifying only the short flag without any argument leads to a value of True.')
parser.add_argument('-s', '--savefigs', dest='save_figures', action='store', nargs='?', const=True, default=False,
                    help='Whether to save the generated plots of distributions. These are saved in figures/stats '
                         'directory. Specifying only the short flag without any argument leads to a value of True.')


def compute_input_stats(rg_iostream=None, print_all=False, make_plots=True, save_figures=False):
